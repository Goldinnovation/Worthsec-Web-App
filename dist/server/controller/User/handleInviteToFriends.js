...
export async function checkForUsersCloseFriends(req, res) {
    try {
...
;
const getCloseFriendsAccData = async (userCloseFriends, req, res) => {
    try {
        const arrobj = userCloseFriends.map((friendsId) => friendsId.userFollowed);
...
export async function inviteClosefriendsToEvent(req, res) {
    try {
