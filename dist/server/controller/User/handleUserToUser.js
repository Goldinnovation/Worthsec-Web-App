...
export async function searchUser_friends(req, res) {
    try {
...
const checksIfUserFollowEachOther = async (userFriendState, req, res) => {
    try {
        const currentUserId = req.user.userId;
        const otherUserId = req.body.userIdData;
        if (userFriendState) {
...
const deleteUserConnection = async (userConnectionId, req, res) => {
    try {
