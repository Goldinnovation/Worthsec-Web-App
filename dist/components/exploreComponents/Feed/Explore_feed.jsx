...
const ExploreFeed = ({ eventDataArr }) => {
    ...
    const handleUserFavoredEvent = async (e) => {
        const FavoredEventReq = await userFavorEvent(e);
    };
    const handleuserJoinedEvent = async (e) => {
        const JoinedEventReq = await userJoinEvent(e);
    };
    return (<div className={styles["explorefeedEvents"]}>
      {eventDataArr?.map((event, i) => (<div key={i} className={styles["exploreContentSection"]}>
          <div className={styles["exploreContent"]} onClick={() => handleToggleitem(event)}>
            <Image src={event?.ImageCoverUpload} width={150} alt="Event cover" height={50} className="rounded-xl h-36
" quality={100}/>
          </div>
          ...
          <div className={styles["exploreContentPopupSection"]}>
            {popupSelectedItem &&
                popupSelectedItem?.eventId === event.eventId && (<div className={styles["exploreContentPopupOverlay"]}>
                  <ExplorePopUpOption eventData={popupSelectedItem} handleToggleitem={handleToggleitem} handleuserJoinedEvent={handleuserJoinedEvent} handleUserFavoredEvent={handleUserFavoredEvent}/>

                  <div className={styles["explorePopUppreview"]}>
                    <ExploreSlider eventArr={eventDataArr} handleToggleitem={handleToggleitem}/>
                  </div>
                </div>)}
          </div>
        </div>))}
